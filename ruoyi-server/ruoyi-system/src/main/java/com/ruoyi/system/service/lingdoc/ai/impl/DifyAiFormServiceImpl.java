...
public class DifyAiFormServiceImpl implements IAiFormService
{
    ...
    public AiExtractResult extract(String filePath, String originalFileName)
    {
        ...
        // 新增：如果文件内容为空，提前返回，避免浪费 Dify Token
        if (StringUtils.isEmpty(fileContent))
        {
            log.warn("文件内容为空，无法识别字段, fileName={}", originalFileName);
            AiExtractResult emptyResult = new AiExtractResult();
            emptyResult.setFields(new ArrayList<>());
            emptyResult.setReferences(new ArrayList<>());
            emptyResult.setTokenCost(0);
            return emptyResult;
        }
        ...
        inputs.put("fileContent", fileContent);
        inputs.put("fileType", fileType);

        log.info("调用 Dify 表格字段识别, fileName={}, contentLength={}", originalFileName, fileContent.length());
        ...
        {
            log.error("Dify 表格识别返回空响应, fileName={}", originalFileName);
            throw new RuntimeException("Dify 表格识别返回空响应");
        }
        if (!"succeeded".equals(resp.getData().getStatus()))
        {
            log.error("Dify 表格识别工作流执行失败, fileName={}, status={}, outputs={}",
                    originalFileName, resp.getData().getStatus(), resp.getData().getOutputs());
        ...
        log.info("Dify 表格字段识别完成, fileName={}, fieldsCount={}, refsCount={}, tokenCost={}",
                originalFileName,
    ...
    public AiGenerateResult generate(String taskId, String originalFilePath, List<LingdocFormField> confirmedFields)
    {
        ...
        log.info("调用 Dify 文档生成, taskId={}, fieldCount={}", taskId, confirmedFields != null ? confirmedFields.size() : 0);
        ...
        if (resp == null || resp.getData() == null)
        {
            log.error("Dify 文档生成返回空响应, taskId={}", taskId);
            throw new RuntimeException("Dify 文档生成返回空响应");
        }
        if (!"succeeded".equals(resp.getData().getStatus()))
        {
            log.error("Dify 文档生成工作流执行失败, taskId={}, status={}, outputs={}",
                    taskId, resp.getData().getStatus(), resp.getData().getOutputs());
        ...
        log.info("Dify 文档生成完成, taskId={}, hasFilePath={}, fieldCount={}, tokenCost={}",
                taskId,
                StringUtils.isNotEmpty(result.getFilledFilePath()),
                result.getFilledValues() != null ? result.getFilledValues().size() : 0,
                result.getTokenCost());
    ...
    /**
     * 读取文件文本内容（支持 HTML/纯文本/docx/pdf/xlsx 等）
     * <p>
     * 使用 Apache Tika 提取二进制文档内容，纯文本文件直接读取。
     */
    private String readFileContent(String filePath, String fileName)
    {
        ...
        // 二进制文件（docx/pdf/xlsx 等）：使用 Apache Tika 提取文本
        List<String> binaryTypes = List.of("docx", "doc", "pdf", "xlsx", "xls", "pptx", "ppt", "rtf", "odt");
        if (binaryTypes.contains(ext.toLowerCase()))
        {
            try
            {
                String content = tika.parseToString(Paths.get(filePath).toFile());
                log.debug("Tika 提取文本成功, file={}, length={}", fileName, content != null ? content.length() : 0);
                return content != null ? content : "";
            }
            catch (Exception e)
            {
                log.warn("Tika 提取文本失败: {}, error={}", filePath, e.getMessage());
                return "";
            }
        }
        ...
        try
        {
            return tika.parseToString(Paths.get(filePath).toFile());
        }
        catch (Exception e)
        {
            log.warn("无法提取文件文本: {}", filePath);
            return "";
        }
    }
