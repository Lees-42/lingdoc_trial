...
| 表名 | 说明 | 记录数预估 | 状态 |
|------|------|-----------|------|
| `lingdoc_file_index` | Vault 文件主索引表 | 单用户 1,000~10,000 条 | ✅ **已创建**（`09-vault-module-mysql.sql`） |

> **重要说明**：`lingdoc_file_index` 表已存在，由 `09-vault-module-mysql.sql` 创建。后续如需调整字段，应编写增量迁移脚本。
...
CREATE TABLE `lingdoc_file_index` (
  ...
  `sub_path`      varchar(512)  DEFAULT NULL COMMENT '子分类路径（相对于documents/，如 学习资料/大三上/操作系统/）',
...
`lingdoc_file_index` 已由 `09-vault-module-mysql.sql` 创建并投入实际使用。当前所有 Vault 接口均依赖该表正常运行。
...
1. **表状态标注**：`lingdoc_file_index` 已由 `09-vault-module-mysql.sql` 创建并投入实际使用
