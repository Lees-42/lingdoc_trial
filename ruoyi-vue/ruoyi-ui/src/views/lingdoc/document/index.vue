        ...
        <el-card>
          ...
          <el-form :inline="true">
            <el-form-item label="选择知识�?>
              <el-select v-model="currentKbId" placeholder="请选择知识�? @change="handleKbChange" style="width: 300px">
                <el-option
                  v-for="kb in kbList"
                  :key="kb.kbId"
                  :label="kb.kbName"
          ...
          <el-table v-loading="loading" :data="docList" v-if="currentKbId">
            <el-table-column label="文件�? prop="originalName" :show-overflow-tooltip="true" />
            ...
            <el-table-column label="页数/字数" width="120">
              <template #default="{ row }">
                <span v-if="row.pageCount">{{ row.pageCount }}�?/span>
                <span v-else-if="row.charCount">{{ row.charCount }}�?/span>
                <span v-else>-</span>
              </template>
            </el-table-column>
            <el-table-column label="状�? prop="processStatusName" width="100">
              <template #default="{ row }">
            ...
            <el-table-column label="分块�? prop="chunkCount" width="80">
              <template #default="{ row }">
          ...
          <!-- 未选择知识库提�?-->
          <el-empty v-if="!currentKbId" description="请先选择知识�? />
...
function getStatusType(status) {
  switch (status) {
    case 0: return 'info'      // 待处�?
    case 1: return 'warning'   // 处理�?
    case 2: return 'success'   // 已完�?
